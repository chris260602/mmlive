...
export const StreamStoreProvider = ({ children }: StreamStoreProviderProps) => {
  ...
  const initApp = async () => {
    ...
    try {
      ...
      const [videoDevices, audioInputs, audioOutputs] = await Promise.all([
        getAllUserVideoInput(),
        getAllUserAudioInput(),
        getAllUserAudioOutput(),
      ]);
      console.log(audioOutputs, "audput");
      ...
      if (audioOutputs.length > 0) {
        ...
        setSelectedAudioOutputDevice(defaultOutput.deviceId);
        setOldAudioOutputDevice(defaultOutput.deviceId);
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    ws.on("new-producer", async ({ producerId, userData, kind }) => {
      ...
      try {
        ...
        if (userData?.id) {
          ...
          setStudentMuteStatus(userData.id, isMuted);
          console.log(
            `Set mute state for new producer ${userData.id}: ${isMuted}`
          );
        }
    ...
    ws.on("mute-student-client", ({ peerId, userId }) => {
      const { userData, peerId: currentPeerId } = userStoreApi.getState();

      if (peerId === currentPeerId && userData!.id === userId) {
        const { stopAudioProduction } = storeRef.current!.getState();
        toast.error("Muted");
        stopAudioProduction();
      }
    });
  ...
  useEffect(() => {
    const unsubscribe = storeRef.current!.subscribe((state, prevState) => {
      // When audio output device changes
      if (
        state.selectedAudioOutputDevice !== prevState.selectedAudioOutputDevice
      ) {
        console.log(
        ...
        if (state.selectedAudioOutputDevice) {
          state.handleAudioOutputChange();
        }
      }
    });
  ...
  useEffect(() => {
    const handleDeviceChange = async () => {
      ...
      try {
        ...
        // ⭐ Check if currently selected audio input still exists
        const currentInputStillExists = audioInputs.some(
          (d) => d.deviceId === selectedAudioDevice
        );

        if (!currentInputStillExists && audioInputs.length > 0) {
          console.warn(
            "⚠️ Current audio input disconnected, switching to default"
          );
          const newDefault = audioInputs[0];
          setSelectedAudioDevice(newDefault.deviceId);
          toast.info(`Microphone changed to: ${newDefault.label || "Default"}`);

          // If mic was active, restart with new device
          const { audioProducerRef, isMicMuted } = storeRef.current!.getState();
          if (audioProducerRef && !isMicMuted) {
            console.log("Restarting audio production with new device");
            await storeRef.current!.getState().handleAudioDeviceChange();
          }
        }

        // ⭐ Check if currently selected audio output still exists
        const currentOutputStillExists = audioOutputs.some(
          (d) => d.deviceId === selectedAudioOutputDevice
        );

        if (!currentOutputStillExists && audioOutputs.length > 0) {
          console.warn(
            "⚠️ Current audio output disconnected, switching to default"
          );
          const newDefault =
            audioOutputs.find((d) => d.deviceId === "default") ||
            audioOutputs[0];
          setSelectedAudioOutputDevice(newDefault.deviceId);
          toast.info(`Speaker changed to: ${newDefault.label || "Default"}`);

          // Apply to all audio elements
          await storeRef
            .current!.getState()
            .applyAudioOutputToAllElements(newDefault.deviceId);
        }

        // ⭐ Notify user about new devices
        const {
          audioDevices: oldAudioInputs,
          audioOutputDevices: oldAudioOutputs,
        } = storeRef.current!.getState();

        // Detect new audio inputs
        const newInputs = audioInputs.filter(
          (device) =>
            !oldAudioInputs.some((old) => old.deviceId === device.deviceId)
        );

        if (newInputs.length > 0) {
          newInputs.forEach((device) => {
            toast.success(
              `New microphone detected: ${device.label || "Unknown"}`
            );
          });
        }

        // Detect new audio outputs
        const newOutputs = audioOutputs.filter(
          (device) =>
            !oldAudioOutputs.some((old) => old.deviceId === device.deviceId)
        );

        if (newOutputs.length > 0) {
          newOutputs.forEach((device) => {
            toast.success(`New speaker detected: ${device.label || "Unknown"}`);
          });
        }

        // Detect removed devices
        const removedInputs = oldAudioInputs.filter(
          (old) =>
            !audioInputs.some((device) => device.deviceId === old.deviceId)
        );

        if (removedInputs.length > 0) {
          removedInputs.forEach((device) => {
            toast.warning(
              `Microphone disconnected: ${device.label || "Unknown"}`
            );
          });
        }

        const removedOutputs = oldAudioOutputs.filter(
          (old) =>
            !audioOutputs.some((device) => device.deviceId === old.deviceId)
        );

        if (removedOutputs.length > 0) {
          removedOutputs.forEach((device) => {
            toast.warning(`Speaker disconnected: ${device.label || "Unknown"}`);
          });
        }
      } catch (error) {
    ...
    return () => {
      ...
      console.log("🗑️ Device change listener removed");
    };
  }, []);
