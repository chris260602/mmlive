...
async function broadcastProducerToRelevantPeers(
  roomName: string,
  producerId: string,
  producerPeer: any,
  kind: string,
  ...
  const peerSocketIds = await redis.smembers(`room:${roomName}:peers`);
  const producerPeerState = await redis.hgetall(`peer:${excludeSocketId}:state`);
  const isMuted = producerPeerState.isMuted !== "0";

  for (const peerSocketId of peerSocketIds) {
    if (peerSocketId === excludeSocketId) continue;

    const peerData = await getPeer(peerSocketId);
    if (!peerData) continue;

    const peerRole = peerData.live_role;
    const producerRole = producerPeer.live_role;

    // Determine if this peer should see this producer
    let shouldNotify = false;

    if (peerRole === "Admin") {
      // Admin sees everything
      shouldNotify = true;
    } else if (peerRole === "Teacher") {
      // Teacher sees only Students
      shouldNotify = producerRole === "Student";
    } else if (peerRole === "Student") {
      // Student sees only Teachers and Admins
      shouldNotify = producerRole === "Teacher" || producerRole === "Admin";
    }

    if (shouldNotify) {
      const socket = io.sockets.sockets.get(peerSocketId);
      if (socket) {
...
export const initializeSocketIO = (io: Server) => {
  ...
  io.on("connection", (socket: any) => {
    ...
    socket.on(
      ...
      async ({ peerId, userId }: { peerId: string; userId: string }) => {
        if (peerId)
          socket.to(peerId).emit("mute-student-client", { peerId, userId });
      }
    );
          ...
          try {
            ...
            if (duplicateProducer) {
              ...
              try {
                ...
                console.log(producer, "PROF");
                await broadcastProducerToRelevantPeers(
                  roomName,
                  producer.id,
                  peer,
                  kind,
                  appData,
                  socket.id,
                  io // You'll need to pass io to the handler
                );
                // await publishToRoom(
                //   roomName,
                //   "producer-closed",
            ...
            const normalizedAppData = {
              ...
              ...(kind === "video" && {
                ...
                // Preserve source if it's screen-share
                ...(appData?.source === "screen-share" && {
                  source: "screen-share",
                }),
              }),
            };
      ...
      async (
        ...
        try {
          ...
          if (existingConsumer) {
            ...
            const producerPeerState = await redis.hgetall(
              `peer:${producerInfo.peerSocketId}:state`
            );
            const isMuted = producerPeerState.isMuted !== "0";
            ...
            return callback({
              params: {
                ...
                userData: { ...producerInfo.peerUserData, isMuted },
