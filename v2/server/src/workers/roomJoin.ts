        ...
        for (const key of producerKeys) {
          ...
          if (producerInfo && producerInfo.peerSocketId) {
            const producerRoom = await redis.get(
              `peer:${producerInfo.peerSocketId}:room`
            );
            if (producerRoom === roomName) {
              ...
              if (producerPeer) {
                const joiningRole = userData.live_role;
                const producerRole = producerPeer.live_role;
                logger.info("JOINING ROLE",userData.live_role)
                let shouldInclude = false;

                if (joiningRole === "Admin") {
                  // Admin sees ALL streams
                  shouldInclude = true;
                } else if (joiningRole === "Teacher") {
                  // Teacher sees only Student streams
                  shouldInclude = producerRole === "Student";
                } else if (joiningRole === "Student") {
                  // Student sees only Teacher and Admin streams
                  shouldInclude =
                    producerRole === "Teacher" || producerRole === "Admin";
                }
                if (shouldInclude) {
                  try {
                    logger.info("mute state",{state:peerState, hasil:(peerState.isMuted !== "0") ? "true" :"false"})
                    const userDataWithState = {
                      ...producerPeer,
                      isMuted: (peerState.isMuted !== "0") ? "true" :"false", // Default to false if not set
                    };
                    producersData.push({
                      producerId,
                    ...
                    logger.warn("Error parsing producer appData during join", {
                      producerId,
                      error: e,
                    });
                  }
